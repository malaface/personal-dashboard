...
export default function FamilyMemberList({ members }: FamilyMemberListProps) {
  ...
  if (members.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-12 text-center">
        <p className="text-gray-500 dark:text-gray-400 text-lg">No hay miembros aún</p>
        <p className="text-gray-400 dark:text-gray-500 mt-2">Agrega tu primer miembro de familia para comenzar</p>
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {members.map((member) => (
        <div key={member.id} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 hover:shadow-md transition">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="p-1 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-md transition"
                title="Edit member"
              >
              ...
              <button
                ...
                className="p-1 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition disabled:opacity-50"
                title="Delete member"
              >
          ...
          <div className="space-y-2">
            ...
            {member.notes && (
              <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{member.notes}</p>
              </div>
            )}
          </div>
        </div>
